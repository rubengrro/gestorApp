...
type Approver = {
  id: string;
  name: string;
  role: string; // Rol del aprobador, como "Supervisor"
};
...
export function SelectorSupervisor({
  selectedApprovers,
  onApproverChange,
  ...
  // Fetch para obtener la lista de supervisores
  React.useEffect(() => {
    const fetchSupervisors = async () => {
      setLoading(true);
      try {
        const response = await fetch("/api/users?role=Supervisor"); // Endpoint para obtener supervisores
  ...
  // Manejar selección y deselección de supervisores
  ...
  // Filtrar supervisores según la búsqueda
      ...
      <>+ Seleccionar supervisores</>
        ...
        <PopoverContent className="p-0 w-full max-w-xs" side="bottom" align="start">
          <Command>
            ...
            <CommandList>
              {loading ? (
                <CommandEmpty>Cargando supervisores...</CommandEmpty>
