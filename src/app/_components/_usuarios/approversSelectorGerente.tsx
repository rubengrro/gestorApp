...
type Approver = {
  id: string;
  name: string;
  role: string; // Rol del aprobador, como "Gerente"
};
...
export function ApproversSelectorGerente({
  selectedApprovers,
  onApproverChange,
  ...
  React.useEffect(() => {
    const fetchGerentes = async () => {
      setLoading(true);
      try {
        const response = await fetch("/api/users?role=Gerente"); // Endpoint para obtener gerentes
        if (!response.ok) throw new Error("Error al cargar gerentes");
