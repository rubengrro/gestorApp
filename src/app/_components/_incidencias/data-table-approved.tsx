...
export const DataTableRegisterApproved = () => {
const { data: session } = useSession();
  ...
  const fetchIncidencias = useCallback(async () => {
    ...
    //       const estadosPermitidos = ["No_Aplica", "Aprobado"];
    //       if (!estadosPermitidos.includes(incidencia.estado)) {
    //         return false; // Excluir incidencias que no tengan el estado permitido
    //       }
    ...
    //       // Los usuarios Superadministrador, Administrador y GPS siempre ven todas las incidencias permitidas
  ...
  const userFilteredIncidencias = useMemo(() => {
    ...
    const filtered = incidencias.filter((incidencia) => {
      // Filtrar solo las incidencias con estado No_Aplica o Aprobado
      const estadosPermitidos = ["No_Aplica", "Aprobado"];
      if (!estadosPermitidos.includes(incidencia.estado)) {
        return false; // Excluir incidencias que no tengan el estado permitido
      }
  ...
  const incidenciaColumns: ColumnDef<IncidenciaForm>[] = [
    ...
    },
    {
      ...
      cell: ({ row }: CellContext<IncidenciaForm, unknown>) => {
        const { monto, cantidad, email, horas } = row.original;
        const value = monto ?? cantidad ?? email ?? horas ?? "-";
        return (
